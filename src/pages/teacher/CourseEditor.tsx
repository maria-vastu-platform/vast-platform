...
export default function CourseEditor() {
    ...
    const handleAddModul = async () => {
        if (!activeCourseId) {
            alert('Bitte zuerst eine Kohorte auswählen.');
            return;
        }
        const title = window.prompt('Modulname:');
        if (!title) return;
        const { error } = await supabase.from('weeks').insert([{ title, order_index: modules.length + 1, course_id: activeCourseId }]);
        if (error) {
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-vastu-text-light mt-1">Module und Lektionen werden in der gewählten Kohorte gespeichert.</p>
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    >
                        {courses.length === 0 && <option value="">Keine Kohorte vorhanden</option>}
            ...
            {!activeCourseId && courses.length === 0 && (
                <div className="bg-white border border-vastu-sand/50 rounded-2xl p-8 text-center text-vastu-text-light">
                    Erstelle zuerst eine Kohorte unter „Kohorten", um Module hinzuzufügen.
                </div>
            )}
